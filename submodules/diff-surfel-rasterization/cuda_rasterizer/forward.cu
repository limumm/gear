	...
	float* __restrict__ out_others)
{
	...
	__shared__ float* collected_mask_logits;  // store mask logits, dynamically allocated
	...
	// Initialize mask blending variables
    float mask_logits_acc[MAX_MASK];
    for (int m = 0; m < nb_mask && m < MAX_MASK; m++) {
		mask_logits_acc[m] = 0.0f;
	}
	
	// dynamically allocate shared memory for mask logits
	extern __shared__ float shared_mem[];
	collected_mask_logits = shared_mem;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			// collect mask logits data (from independent pointer mask_logits)
            for (int m = 0; m < nb_mask && m < MAX_MASK; m++) {
				collected_mask_logits[m * BLOCK_SIZE + block.thread_rank()] = 
                    mask_logits[coll_id * nb_mask + m];
			}
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Mask blending: similar to color blending, but for mask logits
            for (int m = 0; m < nb_mask && m < MAX_MASK; m++) {
				float mask_logit = collected_mask_logits[m * BLOCK_SIZE + j];
				mask_logits_acc[m] += mask_logit * w;
			}
	...
	if (inside)
	{
		...
		// output mask logits to out_others, using uniform offset
        for (int m = 0; m < nb_mask && m < MAX_MASK; m++) {
			out_others[pix_id + (MASK_OFFSET + m) * H * W] = mask_logits_acc[m];
		}
...
void FORWARD::render(
	...
	float* out_others)
{
	renderCUDA<NUM_CHANNELS> << <grid, block, nb_mask * BLOCK_X * BLOCK_Y * sizeof(float) >> > (
		ranges,
		point_list,
		W, H,
		focal_x, focal_y,
