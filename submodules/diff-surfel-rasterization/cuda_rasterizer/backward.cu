...
renderCUDA(
	...
	float* __restrict__ dL_dmask_logits)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
            ...
            // Mask backward: accumulate pixel-level gradients to per-Gaussian mask logits
            // Use normalized contribution: w_norm = w / max(1e-8, (1 - T_final)) to reduce vanishing when alpha blending deep layers
            // Also gate by visibility so that extremely small alphas don't produce noise.
            float denom_T = max(1e-4f, (1.0f - T_final));
            float w_norm = w / denom_T;
            // clamp to avoid exploding gradients when T_final -> 1
            const float W_NORM_CLIP = 10.0f; // can be exposed as a hyper-parameter if needed
            if (w_norm > W_NORM_CLIP) w_norm = W_NORM_CLIP;
            if (alpha >= 1.0f / 255.0f) {
            for (int m = 0; m < nb_mask && m < MAX_MASK; m++) {
                    float g_mask = dL_depths[(MASK_OFFSET + m) * H * W + pix_id];
                    if (g_mask != 0.0f) {
                    atomicAdd(&dL_dmask_logits[global_id * nb_mask + m], w_norm * g_mask);
                    }
                }
            }
		}
...
void BACKWARD::render(
	...
	const int nb_mask,  // parameter for the number of joints
	float * dL_dtransMat,
	float3* dL_dmean2D,
	float* dL_dnormal3D,
	float* dL_dopacity,
	float* dL_dcolors,
	float* dL_dmask_logits)
{
