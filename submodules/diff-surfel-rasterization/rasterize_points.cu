	...
	const bool debug)
{
  ...
  CHECK_INPUT(sh);
  CHECK_INPUT(campos);
  // Allow nb_mask == 0 by skipping device check and using a dummy tensor
  if (nb_mask > 0) {
  ...
  torch::Tensor out_others = torch::full({3+3+1+nb_mask, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
      ...
      // If nb_mask == 0, provide a safe dummy tensor so the pointer is valid even if unused
      torch::Tensor mask_logits_safe;
      if (nb_mask == 0) {
        mask_logits_safe = torch::zeros({1, 1}, float_opts);
      }
      const torch::Tensor& mask_to_use = (nb_mask > 0) ? mask_logits : mask_logits_safe;
      ...
      rendered = CudaRasterizer::Rasterizer::forward(
		...
		mask_to_use.contiguous().data_ptr<float>(),
        nb_mask,
		tan_fovx,
		tan_fovy,
		prefiltered,
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dmask_logits = torch::zeros({P, nb_mask}, means3D.options());
  ...
  {  
	  ...
	  dL_dscales.contiguous().data_ptr<float>(),
	      ...
	      dL_dmask_logits.contiguous().data_ptr<float>(),
	  debug);
  }

  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations, dL_dmask_logits);
}
