...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, mask, gt_alpha_mask, image_name, uid, trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", fid=None, depth=None, mono_depth=None, flow_dirs=[]):
    ...
    def load2device(self, data_device='cuda'):
        ...
        self.mask = self.mask.to(data_device)
        self.gt_alpha_mask = self.gt_alpha_mask.to(data_device)
class MiniCam:
