...
def quaternion_to_axis_angle(quaternions: torch.Tensor):
    ...
    Returns:
        ...
        angle: rotation angles in radians, shape (...,)
    """
    # Ensure float dtype
    ...
    # Separate scalar (real) and vector (imaginary) parts
    w = quaternions[..., 0]  # real part
    xyz = quaternions[..., 1:]  # imaginary part

    # Compute sin(θ/2) and cos(θ/2)
    sin_half_theta = xyz.norm(dim=-1)
    cos_half_theta = w

    # Compute rotation angle θ = 2 * atan2(sin(θ/2), cos(θ/2))
    angle = 2.0 * torch.atan2(sin_half_theta, cos_half_theta)

    # Avoid division by zero (zero rotation → default axis [1,0,0])
    axis = xyz / sin_half_theta.unsqueeze(-1).clamp(min=1e-8)
    axis = torch.where(
        (sin_half_theta < 1e-8).unsqueeze(-1),
        torch.tensor([1.0, 0.0, 0.0], dtype=quaternions.dtype, device=quaternions.device),
        axis
    )
