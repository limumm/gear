...
def load_mesh(path):
    try:
        ...
        print(f"Warning: Failed to load mesh from {path}: {e}")
        print(f"  This may occur when extracted points count is 0 or file does not exist")
        return None, None
...
def compute_recon_error(recon_path, gt_path, n_samples=10000, vis=False):
    ...
    if verts is None or faces is None:
        print(f"Warning: Cannot compute reconstruction error - failed to load reconstruction mesh from {recon_path}")
        return float('inf')
    recon_mesh = Meshes(verts=[verts], faces=[faces])
    
    verts, faces = load_mesh(gt_path)
    if verts is None or faces is None:
        print(f"Warning: Cannot compute reconstruction error - failed to load ground truth mesh from {gt_path}")
        return float('inf')
...
def eval_axis_and_state_all(pred_joint_list, pred_joint_types, gt_info_list, print_perm=False, reverse=False):
    ...
    if num_d_joints <= 1:
        ...
        angle, distance, theta_diff = eval_axis_and_state(pred_joint, gt_joint, gt_joint_types[0],reverse=reverse)
        ...
        for p, perm in enumerate(perms):
            ...
            for i in range(num_d_joints):
                ...
                angle, distance, theta_diff = eval_axis_and_state(pred_joint, gt_joint, gt_joint_types[i],reverse=reverse)
