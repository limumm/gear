...
def post_process_mesh(mesh, cluster_to_keep=1):
    ...
    n_cluster = np.sort(cluster_n_triangles.copy())[-cluster_to_keep] if cluster_to_keep < len(cluster_n_triangles) else 50
...
class GaussianExtractor(object):
    # self, viewpoint_stack, rgbmaps, depthmaps, depth_trunc=5.0, voxel_size=0.004
    def __init__(self, viewpoint_stack, rgbmaps, depthmaps, alphamaps, gaussians, render, pipe, bg_color=None):
        """
        ...
        self.depth_trunc = (self.radius * 5.0)
        self.alphamaps = alphamaps # [1, H, W]
        self.rgbmaps = rgbmaps
        # self.clean()
    ...
    def estimate_bounding_sphere(self):
        ...
        print(f"Use at least {5.0 * self.radius:.2f} for depth_trunc")

    @torch.no_grad()
    def extract_mesh_bounded(self, voxel_size=0.004, sdf_trunc=0.02, mask_backgrond=True):
        """
        ...
        print(f'depth_trunc: {self.depth_trunc}')
        ...
        for i, cam_o3d in tqdm(enumerate(to_cam_open3d(self.viewpoint_stack)), desc="TSDF integration progress"):
            ...
            if mask_backgrond and (self.viewpoint_stack[i].gt_alpha_mask is not None):
                ...
                depth[self.alphamaps[i] < 0.5] = 0

            rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
                o3d.geometry.Image(np.asarray(np.clip(rgb.permute(1,2,0).cpu().numpy(), 0.0, 1.0) * 255, dtype=np.uint8, order="C")),
